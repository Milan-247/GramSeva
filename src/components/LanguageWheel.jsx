  ...
  return (
    <div className={`relative flex items-center bg-black/40 backdrop-blur-md rounded-2xl border border-white/20 p-1 shadow-inner max-w-full overflow-hidden group ${className}`}>
      ...
      <button
        type="button"
        onClick={handlePrev}
        className="shrink-0 w-7 h-7 flex items-center justify-center rounded-xl text-amber-200/80 hover:text-white hover:bg-white/15 active:scale-95 transition z-20"
      ...
      <div className="pointer-events-none absolute left-7 inset-y-0 w-5 bg-gradient-to-r from-[#0e1626]/90 via-[#0e1626]/40 to-transparent z-10" />
      <div className="pointer-events-none absolute right-7 inset-y-0 w-5 bg-gradient-to-l from-[#0e1626]/90 via-[#0e1626]/40 to-transparent z-10" />
        ...
        {supportedLanguages.map((lang) => {
          ...
          return (
            <button
              ...
              } rounded-xl font-bold transition-all duration-200 cursor-pointer outline-none focus-visible:ring-2 focus-visible:ring-amber-400 ${
                isActive
                  ? "text-white font-black scale-105 shadow-md"
                  : "text-amber-100/70 hover:text-white hover:scale-100 opacity-75 hover:opacity-100"
              }`}
            >
              ...
              {isActive && (
                <motion.div
                  ...
                  className="absolute inset-0 bg-gradient-to-r from-[#c26111] via-[#e07a1e] to-[#c26111] rounded-xl shadow-lg border border-amber-300/40"
              ...
              <span
                className={`relative z-10 text-[9px] font-mono font-bold uppercase px-1 py-0.5 rounded ${
                  isActive ? "bg-black/30 text-amber-100" : "bg-white/10 text-amber-200/80"
                }`}
              >
                {lang.code}
              </span>
