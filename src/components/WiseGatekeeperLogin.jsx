              ...
              {authMode === "register" && (
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1">
                  ...
                  <div>
                    ...
                    >
                      {activeDistricts.map((d, idx) => {
                        const distName = typeof d === "string" ? d : (d.en || d.id);
                        const distKey = typeof d === "string" ? d : (d.id || d.en);
                        return (
                          <option key={`dist_${distKey}_${idx}`} value={distName} className="bg-white text-slate-900">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-transparent border-b border-stone-300 text-slate-900 py-2 text-xs font-medium outline-none focus:border-slate-900 cursor-pointer"
                    >
                      {(activePanchayats[district] || [{ en: locality, ml: locality }]).map((p, idx) => {
                        const pName = typeof p === "string" ? p : (p.en || p.ml);
                        const pCode = typeof p === "object" && p.code ? p.code : (p.en || p.ml || pName);
                        return (
                          <option key={`p_${pCode}_${idx}`} value={pName} className="bg-white text-slate-900">
